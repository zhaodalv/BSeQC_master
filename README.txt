...
Bisulfite sequencing (BS-seq) has been extensively used for DNA methylome study. Ideally, BS-seq experiment should be able to directly and exactly identify the methylation state of a DNA fragment from the original genome. However, current BS-seq protocols still possess several intrinsic biases, which will change the methylation state of cytosines from the original genome, such as overhang end-repair bias, five-prime bisulfite non-conversion bias, sequencing into the adaptor bias and base-call qualities bias.  Since BS-seq experiments are widely used and the resulting data will continue to grow exponentially in the near future, there is a strong need for a dedicated QC tool to access and improve the quality of BS-seq experiments. Here, we developed BSeQC package. It is exclusively focused on the comprehensive assessment of BS-seq specific biases. And it can automatically evaluate and remove the biases based on user specified cutoff. In addition, BSeQC also support removing duplicate reads and keeping one copy of the overlapping segment in paired-end sequencing. 
