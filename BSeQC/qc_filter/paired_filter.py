...
def read_trim_paired(read, strand_t, out, read_info, original_length, auto, remove_overlap, duplicate,
                     filter_mbias_trim_bp, filter_duplicate_reads, filter_remove_overlap_bp):
    '''
    ...
    if duplicate:
        ...
        return filter_mbias_trim_bp, filter_duplicate_reads, filter_remove_overlap_bp
    ...
    pos1_new, pos2_new, CIGAR, insert, seq, score, filter_mbias_trim_bp, filter_remove_overlap_bp = get_trimmed_position(
        strand_t, strand, readlen, p_len,
        pos1, pos2, seq, score, CIGAR, remove_overlap, filter_mbias_trim_bp, filter_remove_overlap_bp)
    if len(seq) == 0:
        return filter_mbias_trim_bp, filter_duplicate_reads, filter_remove_overlap_bp
    else:
...
def get_trimmed_position(strand_t, strand, readlen, p_len, pos1, pos2, seq, score, CIGAR, remove_overlap,
                         filter_mbias_trim_bp, filter_remove_overlap_bp):
    #get the trimming positions
    #using the other paired read to decide the new pos2 and insert size
    ...
    return pos1_new, pos2_new, CIGAR, insert, seq, score, filter_mbias_trim_bp, filter_remove_overlap_bp
